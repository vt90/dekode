...
const getAddressBalance = async (address) => {
  ...
  try {
    ...
    const receivedTransactions = await getToTransactions(address);
    const sentTransactions = await getFromTransactions(address);

    receivedTransactions.forEach((trs) => {
      total_received += trs.value;
      final_balance += trs.value
    });

    sentTransactions.forEach((trs) => {
      total_sent += trs.value;
      final_balance -= trs.value
    });

    const final_n_tx = sentTransactions.length + receivedTransactions.length;
...
const getTransactionsByAddress = async (address) => {
  const url = `https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=20000&sort=asc&apikey=${ETH_API_KEY}`;
